...
if [ `grep -c 'targetSdkVersion 29' .android/app/build.gradle` -eq '1' ]; then
     echo "targetSDK 已经是26 ，不需要修改"
else
     echo "targetSDK 需要修改为 29 "
     sed -i '' 's/targetSdkVersion 28/targetSdkVersion 29/g' .android/app/build.gradle
fi

echo "扫描 Flutter/build.gradle 中 targetSDK "
if [ `grep -c 'targetSdkVersion 29' .android/Flutter/build.gradle` -eq '1' ]; then
     echo "targetSDK 已经是26 ，不需要修改"
else
     echo "targetSDK 需要修改为 29 "
     sed -i '' 's/targetSdkVersion 28/targetSdkVersion 29/g' .android/Flutter/build.gradle
fi
